...
import { Search, Filter, Eye, FileText, MessageCircle, Pencil, Trash2, Truck, RotateCcw, Package, UserCheck, Scale, StickyNote } from "lucide-react";
...
export default function GTOrdersPage() {
    ...
    const loadOrders = useCallback(async (mounted = true) => {
        ...
        try {
            ...
            if (!mounted) return;
            // Filter to only SALES_GT orders
            const gtOrders = (all || []).filter(o => o.source === 'SALES_GT');
            setOrders(gtOrders);

            if (gtOrders.length > 0) {
                const unread = await getOrdersWithUnreadMessages(gtOrders.map(o => o.id), session?.access_token);
                if (mounted) setUnreadOrders(unread);
            }
        } catch (error) {
            console.error("[GTOrders] Error:", error);
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('gt_orders_realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, () => loadOrders(mounted))
            .subscribe();

        const chatChannel = supabase
            .channel('gt_chat_realtime')
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'order_messages' }, (payload: any) => {
                const newMsg = payload.new as any;
                if (newMsg?.order_id) {
                    setUnreadOrders(prev => { const next = new Set(prev); next.add(newMsg.order_id); return next; });
                }
            })
            .subscribe();
                                ...
                                return (
                                    <React.Fragment key={order.id}>
                                        <tr className="hover:bg-slate-50">
                                            <td className="px-6 py-4 font-medium text-slate-900">ORD-{order.readableId}</td>
                                            <td className="px-6 py-4 text-slate-600">{formatDate(order.createdAt)}</td>
                                            <td className="px-6 py-4">
                                                <div className="font-medium text-slate-900">{order.customerName}</div>
                                            </td>
                                            <td className="px-6 py-4 text-right font-medium text-slate-900">{formatPrice(order.totalAmount)}</td>
                                            ...
                                            <tr className="bg-blue-50/40 border-b border-slate-100">
                                                <td colSpan={6} className="px-6 py-2">
                                                    <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 text-xs">
                                                        ...
                                                        {order.packedByName && (
                                                            <span className="inline-flex items-center gap-1 text-slate-600">
                                                                <UserCheck className="w-3 h-3 text-emerald-500" /> Đóng: <strong>{order.packedByName}</strong>
                                                            </span>
                                                        )}
                                                        {order.totalBoxes && order.totalBoxes > 0 ? (
                                                            <span className="inline-flex items-center gap-1 text-slate-600">
                                                                <Package className="w-3 h-3 text-amber-500" /> {order.totalBoxes} thùng
                                                        ...
                                                        {order.shippingNote && (
                                                            <span className="inline-flex items-center gap-1 text-slate-500 italic">
                                                                <StickyNote className="w-3 h-3 text-slate-400" /> {order.shippingNote}
                                                            </span>
                                                        )}
                                                    </div>
                                                </td>
