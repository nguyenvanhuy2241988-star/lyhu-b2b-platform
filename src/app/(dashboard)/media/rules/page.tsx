...
const DEFAULT_POLICY: PolicyData = {
    ...
    allowances: [
        { name: "Di chuyển", amount: "500.000đ" },
        { name: "Điện thoại", amount: "100.000đ" },
        { name: "Bảo trì thiết bị", amount: "200.000đ" },
    ],
    bonuses: [
        { title: "Hoàn thành dự án trước deadline", amount: "+200.000đ", desc: "Mỗi dự án hoàn thành trước hạn" },
        { title: "Ảnh/Video được chọn làm chiến dịch", amount: "+300.000đ", desc: "Sản phẩm được phòng Marketing sử dụng" },
        { title: "Sáng kiến cải tiến", amount: "+100 - 300k", desc: "Ý tưởng concept, góc quay mới hiệu quả" },
    ],
    penalties: [
        { name: "Đi muộn / Về sớm", desc: "Không có lý do chính đáng & chưa báo Admin", fine: "50.000đ" },
        { name: "Hư hỏng thiết bị do tắc trách", desc: "Không bảo quản đúng quy trình", fine: "Theo giá trị" },
        { name: "Giao muộn so với deadline", desc: "Chậm deadline đã cam kết mà không báo trước", fine: "100.000đ" },
    ],
    penaltyNote: "Mọi khoản phí phạt đều được gom vào quỹ Bonding để dùng cho các hoạt động ngoại khóa, liên hoan của tập thể.",
    commissionNote: "Nhân viên Media được hưởng thưởng dựa trên số lượng dự án hoàn thành và chất lượng sản phẩm.",
    version: "v1.0"
};
...
export default function MediaRulesPage() {
    ...
    const loadPolicy = useCallback(async () => {
        ...
        if (policies.media_creator) {
            setPolicy({ ...DEFAULT_POLICY, ...policies.media_creator });
        }
    }, []);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            policies.media_creator = editPolicy;
    ...
    return (
        <div className="max-w-3xl mx-auto py-8 px-4 space-y-8">
            <div className="flex items-center justify-between">
                ...
                {isAdmin && !isEditing && (
                    <button onClick={openEdit} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-pink-600 hover:bg-pink-700 rounded-lg transition-colors">
            ...
            <div>
                <h1 className="text-xl font-bold text-slate-900">Chính sách Thu nhập Media (Ảnh/Video)</h1>
                <p className="text-sm text-slate-500 mt-1">Minh bạch, công bằng và tập trung vào chất lượng sản phẩm.</p>
            </div>
            ...
            <section className="bg-white rounded-xl border border-slate-200">
                ...
                <div className="p-5 space-y-4">
                    <div className="flex items-baseline justify-between">
                        <div>
                            <p className="text-xs text-slate-400 font-medium mb-1">Mức lương Media</p>
                    ...
                    <div className="bg-white rounded-lg w-full max-w-2xl shadow-lg overflow-hidden max-h-[90vh] flex flex-col">
                        <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                            <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                                <Settings className="w-5 h-5 text-pink-500" /> Chỉnh sửa Chính sách Media
                            </h2>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <button onClick={() => setEditPolicy({ ...editPolicy, allowances: [...editPolicy.allowances, { name: "", amount: "" }] })}
                                    className="text-xs text-pink-600 flex items-center gap-1"><Plus className="w-3 h-3" /> Thêm phụ cấp</button>
                            </div>
                            <div className="space-y-3">
                                ...
                                <button onClick={() => setEditPolicy({ ...editPolicy, bonuses: [...editPolicy.bonuses, { title: "", amount: "", desc: "" }] })}
                                    className="text-xs text-pink-600 flex items-center gap-1"><Plus className="w-3 h-3" /> Thêm mục thưởng</button>
                            </div>
                            <div className="space-y-3">
                                ...
                                <button onClick={() => setEditPolicy({ ...editPolicy, penalties: [...editPolicy.penalties, { name: "", desc: "", fine: "" }] })}
                                    className="text-xs text-pink-600 flex items-center gap-1"><Plus className="w-3 h-3" /> Thêm chế tài</button>
                            </div>
                            <div className="space-y-3">
                                <h3 className="text-xs font-bold text-slate-500 uppercase">Ghi chú thưởng</h3>
