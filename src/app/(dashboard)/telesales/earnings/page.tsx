...
export default function TelesalesEarningsPage() {
    ...
    if (allTasks[5]?.data) setKpiTracking(allTasks[5].data); // The result of get_telesales_kpi_tracking is the 6th element (allTasks is 0, orders 1, txs 2, config 3, kpiData 4, tracking 5)
    // Wait, Promise.all order:
    // 0: tasks
    // 1: orders
    // 2: transactions
    // 3: config
    ...
    // Correction below:
    if (allTasks && Array.isArray(allTasks)) setTasks(allTasks as any); // Type fix if needed
    // Actually promise returns array of results.
    // let [tasksRes, ordersRes, txRes, configRes, settingsRes, trackingRes] = ...
    // I'm using array destructuring but sticking to the existing variable names which makes it confusing.

    // Let's rewrite the set part cleanly.
