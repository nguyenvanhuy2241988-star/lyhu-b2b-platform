...
// Department key used to load/save from app_settings.income_policies
const DEPT_KEY = 'recruiter';
const DEPT_LABEL = 'Tuyển dụng (HR)';

export default function RecruiterRulesPage() {
    ...
    const loadPolicy = useCallback(async () => {
        ...
        if (policies[DEPT_KEY]) {
            setPolicy({ ...DEFAULT_POLICY, ...policies[DEPT_KEY] });
        }
    }, []);
    ...
    const hasPolicy = policy.baseSalary > 0 || policy.bonuses.length > 0 || policy.penalties.length > 0;
    ...
    return (
        <div className="max-w-3xl mx-auto py-8 px-4 space-y-8">
            ...
            {/* Header */}
            <div>
                <h1 className="text-xl font-bold text-slate-900">Chính sách Thu nhập {DEPT_LABEL}</h1>
            ...
            {!hasPolicy ? (
                <div className="bg-slate-50 rounded-xl border border-slate-200 p-12 text-center space-y-4">
                    <div className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center mx-auto">
                        <Info className="w-6 h-6 text-slate-400" />
                    </div>
                    <div>
                        <p className="text-sm font-medium text-slate-700">Chưa có chính sách thu nhập cho {DEPT_LABEL}</p>
                        <p className="text-xs text-slate-400 mt-1">Admin có thể thiết lập chính sách từ trang <strong>Lương & Thưởng</strong> hoặc bấm nút "Chỉnh sửa chính sách" phía trên.</p>
                    </div>
                </div>
            ) : (
                <>
                    ...
                    <section className="bg-white rounded-xl border border-slate-200">
                        ...
                        <div className="p-5 space-y-4">
                            <div className="flex items-baseline justify-between">
                                <div>
                                    <p className="text-xs text-slate-400 font-medium mb-1">Mức lương</p>
