...
export default function EventsCulturePage() {
    ...
    // Transaction Modal (add/edit) — admin only
    ...
    const refreshBalance = useCallback(() => { getFundBalance().then(setBalance); }, []);
    const refreshTransactions = useCallback(() => { getFundTransactions().then(setTransactions); }, []);
    const refreshProfiles = useCallback(() => { getHRProfiles().then(setAllProfiles); }, []);
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const [transData, balanceData, profiles] = await Promise.all([
                    getFundTransactions(),
                    getFundBalance(),
                    getHRProfiles()
                ]);
    ...
    useEffect(() => {
        const channel = supabase
            .channel('events-culture-realtime')
    ...
    const openEditTransaction = (t: FundTransaction) => {
        setEditingTrans(t);
        setNewTrans({ description: t.description, amount: String(t.amount), type: t.type, category: t.category || 'Khác' });
        setIsTransModalOpen(true);
    };
        ...
        try {
            if (editingTrans) {
                await updateFundTransaction(editingTrans.id, {
                    description: newTrans.description, amount: Number(newTrans.amount),
                    type: newTrans.type as 'income' | 'expense', category: newTrans.category
                });
            } else {
            ...
            setIsTransModalOpen(false); setEditingTrans(null);
        } catch (error) { console.error(error); alert("Lỗi lưu giao dịch"); }
    };
    const handleDeleteTransaction = async (id: string) => {
        try { await deleteFundTransaction(id); refreshTransactions(); refreshBalance(); setDeletingTransId(null); }
        catch (e) { console.error(e); alert("Lỗi xóa giao dịch"); }
    };
    const handleConfirmContribution = async (contribId: string) => {
        if (!user?.id) return;
        try { await confirmFundContribution(contribId, user.id); getFundContributions(contribMonth, contribYear).then(setContributions); }
        catch (e) { console.error(e); alert("Lỗi xác nhận"); }
    };
    const handleMarkPaid = async (userId: string) => {
        try { await upsertFundContribution(userId, contribMonth, contribYear, 'confirmed'); getFundContributions(contribMonth, contribYear).then(setContributions); }
    ...
    const contributionList = useMemo(() => {
        return allProfiles.map(p => {
            ...
            return {
                userId: p.id, fullName: p.full_name, avatarUrl: p.avatar_url,
                status: contrib?.status || 'pending', contribId: contrib?.id, confirmedAt: contrib?.confirmed_at
            };
        });
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="bg-white rounded-xl border border-slate-200 p-5">
                            <p className="text-xs text-slate-400 mb-1">Số dư quỹ</p>
                            <p className={`text-2xl font-bold ${balance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{fmt(balance)}</p>
                        </div>
                        <div className="bg-white rounded-xl border border-slate-200 p-5">
                            <p className="text-xs text-slate-400 mb-1">Nhân sự đóng</p>
                            <p className="text-2xl font-bold text-slate-900">{fmt(monthlyPerPerson)}<span className="text-sm font-normal text-slate-400">/người</span></p>
                        </div>
                        <div className="bg-white rounded-xl border border-slate-200 p-5">
                            <p className="text-xs text-slate-400 mb-1">Công ty đóng thêm</p>
                            <p className="text-2xl font-bold text-slate-900">{fmt(monthlyCompany)}</p>
                        </div>
                    </div>
                    ...
                    <section className="bg-white rounded-xl border border-slate-200">
                        <div className="px-5 py-4 border-b border-slate-100 space-y-3">
                            <div className="flex items-center justify-between">
                                ...
                                {isAdmin && (
                                    <button onClick={openAddTransaction} className="flex items-center gap-1 text-xs text-teal-600 hover:text-teal-700 font-medium">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        {isAdmin && (
                                            <div className="hidden group-hover:flex items-center gap-1">
                                                <button onClick={() => openEditTransaction(t)} className="p-1 text-slate-400 hover:text-teal-600 rounded"><Pencil className="w-3 h-3" /></button>
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {isAdmin && c.contribId && (
                                                    <button onClick={() => handleConfirmContribution(c.contribId!)}
                                                        className="text-xs text-teal-600 hover:text-teal-700 font-medium">Xác nhận</button>
                                                )}
                                            </div>
                                        ) : (
                                            <div className="flex items-center gap-2">
                                                ...
                                                {isAdmin && (
                                                    <button onClick={() => handleMarkPaid(c.userId)}
                                                        className="text-xs text-teal-600 hover:text-teal-700 font-medium">Đánh dấu đã đóng</button>
                                                )}
                                            </div>
                                        )}
                                    </div>
                    ...
                    <div className="bg-white rounded-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-5 space-y-4">
                            <div>
                                ...
                                <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-teal-400 transition-colors"
                            ...
                            <div>
                                ...
                                <input className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-teal-400 transition-colors"
                        ...
                        <div className="px-5 pb-5">
                            <button onClick={saveBankConfig} className="w-full py-2.5 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700 transition-colors">Lưu cài đặt</button>
                        </div>
                    </div>
                </div>
            )}

            {/* ===================== TRANSACTION MODAL (Admin) ===================== */}
            {isTransModalOpen && (
                    ...
                    <div className="bg-white rounded-xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="number" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-teal-400 transition-colors" placeholder="0"
                                    value={newTrans.amount} onChange={e => setNewTrans({ ...newTrans, amount: e.target.value })} />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-slate-500 mb-1.5">Nội dung</label>
                                <input className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-teal-400 transition-colors" placeholder="VD: Tiệc tất niên"
                            ...
                            <div>
                                ...
                                <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-teal-400 transition-colors"
                                    ...
                                    <option value="Ăn uống">Ăn uống</option><option value="Sự kiện">Sự kiện</option>
                                    <option value="Sinh nhật">Sinh nhật</option><option value="Team building">Team building</option>
                                    <option value="Quà tặng">Quà tặng</option><option value="Đóng quỹ">Đóng quỹ</option>
                                    <option value="Khác">Khác</option>
                                </select>
                            </div>
                        </div>
                        <div className="px-5 pb-5">
                            <button onClick={handleSaveTransaction} className="w-full py-2.5 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700 transition-colors">
                                {editingTrans ? 'Lưu thay đổi' : 'Thêm giao dịch'}
                            </button>
                        </div>
                    </div>
                </div>
