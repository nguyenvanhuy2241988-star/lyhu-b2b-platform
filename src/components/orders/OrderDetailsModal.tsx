...
export function OrderDetailsModal({ order, isOpen, onClose }: OrderDetailsModalProps) {
    ...
    export function OrderDetailsModal({ order, onClose }: OrderDetailsModalProps) {
        ...
        const handleDownloadImage = async () => {
            ...
            // Force show it, remove print-specific hiding for canvas capture
            element.className = 'p-8 bg-white text-black font-sans w-[800px] mx-auto';
            // We need to append it to body or make sure it's rendered visibly for html2canvas
            // But since it's already in the DOM (just hidden by css), changing className might work if parent isn't hidden.
            // Better strategy: Capture the ref directly. 
            // Since it has 'hidden print:block', it's display:none on screen. html2canvas won't capture display:none.

            // Helper to clone and render for capture
            // Actually, let's just cheat: Change styles, capture, revert.
