            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                        ...
                                        <React.Fragment key={order.id}>
                                            ...
                                            {hasShippingData && (
                                                <tr className="bg-blue-50/40 border-b border-slate-100">
                                                    <td colSpan={hideRevenue ? 7 : 8} className="px-6 py-2">
                                                                ...
                                                                <span className="inline-flex items-center gap-1 text-slate-600">
                                                                    ...
                                                                    {order.shippingBoxes && order.shippingBoxes.length > 0 && (
                                                                        <span className="text-slate-400 ml-0.5">
                                                                            ({order.shippingBoxes.map((b: any, i: number) => `${b.length_cm || 0}x${b.width_cm || 0}x${b.height_cm || 0}cm`).join(', ')})
                                                                        </span>
                                                                    )}
                                                                </span>
                                                            ) : null}
                ...
                <div className="lg:hidden divide-y divide-slate-100">
                    {isLoading ? (
                        ...
                        <div className="p-8 text-center text-slate-500">Không tìm thấy đơn hàng nào</div>
                    ) : (
                        filteredOrders.map((order) => {
                            const normalizedStatus = (order.status || "pending").toLowerCase() as keyof typeof STATUS_CONFIG;
                            const statusConfig = STATUS_CONFIG[normalizedStatus] || STATUS_CONFIG.pending;
                            ...
                            return (
                                <div key={order.id} className={`p-4 bg-white transition-colors ${isCompleted ? "opacity-80" : ""} ${selectedIds.has(order.id) ? 'bg-primary-50/50' : 'hover:bg-slate-50'}`}>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <button onClick={() => setChatOrder({ id: order.id, readableId: String(order.readableId || order.id.slice(0, 8)) })} className="relative flex-1 min-w-[70px] flex items-center justify-center gap-1.5 bg-slate-50 text-slate-700 hover:bg-slate-100 py-2.5 rounded-lg text-sm font-semibold transition-colors border border-slate-200">
                                            <MessageCircle className="w-4 h-4" /> Chat
                                            {unreadOrders.has(order.id) && <span className="absolute top-1.5 right-2 w-2 h-2 bg-red-500 rounded-full" />}
                                        </button>
                                        <button onClick={() => setSelectedOrder(order)} className="flex-1 min-w-[70px] flex items-center justify-center gap-1.5 bg-slate-50 text-slate-700 hover:bg-slate-100 py-2.5 rounded-lg text-sm font-semibold transition-colors border border-slate-200">
                                            <Eye className="w-4 h-4" /> Chi tiết
                                        </button>
                                        <button onClick={() => setPrintOrder(order)} className="flex-1 min-w-[70px] flex items-center justify-center gap-1.5 bg-slate-50 text-slate-700 hover:bg-slate-100 py-2.5 rounded-lg text-sm font-semibold transition-colors border border-slate-200">
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        {!readOnly && order.status === 'pending' && (
                                            <button onClick={() => setEditOrder(order)} className="flex-1 flex items-center justify-center gap-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 py-2.5 rounded-lg text-sm font-semibold transition-colors">
                                                <Edit className="w-4 h-4" /> Sửa đơn
                                            </button>
                                        )}
                                        {!readOnly && order.status !== 'pending' && canEditShipping && (
                                            <button onClick={() => setShippingEditOrder(order)} className="flex-1 flex items-center justify-center gap-1.5 bg-purple-50 text-purple-600 hover:bg-purple-100 py-2.5 rounded-lg text-sm font-semibold transition-colors">
                                                <Truck className="w-4 h-4" /> Sửa VC
                                            </button>
                                        )}
                                        {readOnly && canEditShipping && (
                                            <button onClick={() => setShippingEditOrder(order)} className="flex-1 flex items-center justify-center gap-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 py-2.5 rounded-lg text-sm font-semibold transition-colors">
                                                <Truck className="w-4 h-4" /> Sửa VC
                                            </button>
                                        )}
                                        {!readOnly ? (
                                            ...
                                            <div className="flex-1 px-3 py-2.5 bg-slate-100 text-slate-500 text-sm font-medium rounded-lg text-center border border-slate-200">
                                                Chỉ xem
                                            </div>
                                        )}
                                    </div>
                                    
                                    {role !== 'warehouse' && !readOnly && (
