            ...
            <div className="p-4 grid grid-cols-2 gap-2 border-b">
                ...
                <button
                    onClick={() => {
                        ...
                        // Prefer fb_thread_id (accurate), fallback to external_id (PSID)
                        const selectedId = conversation.fb_thread_id || conversation.external_id;
                        const url = pageId && selectedId
                            ? `https://business.facebook.com/latest/inbox/all?asset_id=${pageId}&selected_item_id=${selectedId}&mailbox_id=&thread_type=FB_MESSAGE`
                            : pageId
