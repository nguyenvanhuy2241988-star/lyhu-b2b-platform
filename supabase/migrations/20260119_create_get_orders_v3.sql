-- Create V3 to cleanly replace the ambiguous v2 versions
CREATE OR REPLACE FUNCTION public.get_orders_v3(
    p_id uuid DEFAULT NULL, -- Added to support fetching single order by ID
...
BEGIN
    ...
    WHERE 
        ...
        -- 2. FILTERS
        AND (p_id IS NULL OR o.id = p_id) -- Filter by single ID
