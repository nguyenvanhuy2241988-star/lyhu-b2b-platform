...
drop policy if exists "Admin manage user_achievements" on user_achievements;
CREATE POLICY "Admin manage user_achievements" ON user_achievements FOR ALL USING (
    EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin')
);
drop policy if exists "Admin manage career_levels" on career_levels;
