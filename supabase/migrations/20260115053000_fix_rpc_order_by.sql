-- Fix: Wrap UNION in subquery to avoid ORDER BY error
-- Date: 2026-01-15
...
BEGIN
    ...
    SELECT * FROM (
        ...
        AND (
            ...
            OR EXISTS (SELECT 1 FROM unnest(t.assignee_ids) aid WHERE aid::text = p_user_id::text)
        )
    ) as unified_result
    ORDER BY unified_result.due_date ASC;
END;
$$;
