-- Migration: Update KPI RPCs to use Orders for Revenue (Deduct VAT)
-- Date: 2026-02-05
-- Description: Switches Revenue and Won Deal calculation to use 'orders' table (delivered).
--              DEDUCTS VAT from Total Revenue.
...
BEGIN
    ...
    -- Deduct VAT from total_amount
    SELECT COALESCE(SUM(total_amount - COALESCE(vat, 0)), 0), COUNT(*)
