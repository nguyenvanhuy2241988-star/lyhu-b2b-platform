-- Migration: Add geographic location tracking to analytics
-- Description: Adds city and region columns to website_page_views for province/city analytics.

-- Add columns (country already exists from original schema but was never populated)
ALTER TABLE public.website_page_views ADD COLUMN IF NOT EXISTS city text;
ALTER TABLE public.website_page_views ADD COLUMN IF NOT EXISTS region text;

-- Index for fast region-based queries
CREATE INDEX IF NOT EXISTS idx_website_page_views_region ON public.website_page_views (region);

-- Update RPC Function to include geo stats
...
DECLARE
    ...
    top_regions jsonb; top_cities jsonb;
    result jsonb;
BEGIN
    ...
    -- Top Regions / Provinces (Human only)
    SELECT jsonb_agg(row_to_json(t)) INTO top_regions FROM (
        SELECT COALESCE(NULLIF(region, ''), 'Không xác định') as name, COUNT(*) as views, COUNT(DISTINCT visitor_id) as visitors
        FROM public.website_page_views
        WHERE created_at >= start_date AND created_at <= end_date AND (is_bot IS NULL OR is_bot = false)
        AND (NOT exclude_internal OR pathname NOT SIMILAR TO '/(admin|crm|hr|recruitment|marketing|media|sales-kit|telesales|documents|accountant)%')
        GROUP BY COALESCE(NULLIF(region, ''), 'Không xác định') ORDER BY views DESC LIMIT 15
    ) t;

    -- Top Cities (Human only)
    SELECT jsonb_agg(row_to_json(t)) INTO top_cities FROM (
        SELECT COALESCE(NULLIF(city, ''), 'Không xác định') as name, COALESCE(NULLIF(region, ''), '') as region, COUNT(*) as views
        FROM public.website_page_views
        WHERE created_at >= start_date AND created_at <= end_date AND (is_bot IS NULL OR is_bot = false)
        AND (NOT exclude_internal OR pathname NOT SIMILAR TO '/(admin|crm|hr|recruitment|marketing|media|sales-kit|telesales|documents|accountant)%')
        GROUP BY COALESCE(NULLIF(city, ''), 'Không xác định'), COALESCE(NULLIF(region, ''), '') ORDER BY views DESC LIMIT 15
    ) t;
    ...
    result := jsonb_build_object(
        ...
        'topRegions', COALESCE(top_regions, '[]'::jsonb),
        'topCities', COALESCE(top_cities, '[]'::jsonb)
    );
    RETURN result;
END;
$$;
